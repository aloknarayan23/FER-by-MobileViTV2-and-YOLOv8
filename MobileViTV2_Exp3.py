...
# Freezing all layers of the pretrained model.
for name, param in model.named_parameters():
    param.requires_grad = False

# Defining the modified model with custom layers and classification head
class CustomMobileViT(nn.Module):
    def __init__(self, original_model, num_classes):
        super(CustomMobileViT, self).__init__()
        self.features = original_model.mobilevitv2.encoder  # Exclude the original classifier
        self.custom_layer = nn.Sequential(
            nn.Conv2d(960, 512, kernel_size=1),  # Example custom layer
            nn.ReLU()
        )
        self.classifier = nn.Sequential(
            nn.Flatten(),
            nn.Linear(512 * 8 * 8, 256),  # Adjust input features if necessary
            nn.ReLU(),
            nn.Dropout(0.5),
    ...
    def forward(self, x):
        x = self.features(x)
        print(f"Features shape: {x.shape}")  # Print shape after feature extraction
        x = self.custom_layer(x)
        print(f"Custom layer output shape: {x.shape}")  # Print shape after custom layer
        x = self.classifier(x)
        print(f"Classifier output shape: {x.shape}")  # Print shape after classifier
        return x
...
model = CustomMobileViT(model, num_classes=num_labels)
...
joblib.dump(model, 'MViTV2_Exp3.pkl') 
...
plt.savefig('MViTV2_Exp3_confusion_matrix.png')  
...
with open('MViTV2_Exp3_classification_report.txt', 'w') as f:
    f.write(report)
